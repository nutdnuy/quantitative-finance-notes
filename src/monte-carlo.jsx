...
export default function MonteCarloLab(){
  ...
  const d=useMemo(()=>{
    const paths=simulate({mu:mu/100,sigma:sigma/100,seed,count,steps:252,initial:100});
    ...
    const visible=paths.slice(0,300),maxPrice=visible.reduce((m,p)=>Math.max(m,...p),theory)*1.03,minPrice=visible.reduce((m,p)=>Math.min(m,...p),100)*0.97;
    return {terminal,average,se,theory,bins,convergence,visible,maxPrice,minPrice,loss:terminal.filter(v=>v<100).length/count*100};
  },[mu,sigma,count,seed]);
  return <div className="lab"><LabTitle number="เพิ่มเติม" title="Monte Carlo: ลองสุ่มอนาคตหลาย ๆ ครั้ง">หุ้นวันนี้ 100 ดอลลาร์ · ระยะเวลา 1 ปี · ใช้ GBM ตามสมมติฐานที่เลือก</LabTitle>
    ...
    <Chart tall title="Monte Carlo: เส้นทางราคาสุ่มหลายสี" description={`แสดง ${d.visible.length} เส้นจากทั้งหมด ${count} เส้น ทุกเส้นเริ่มที่ 100 ดอลลาร์ สีใช้แยกเส้นทางเท่านั้น`} yDomain={[d.minPrice,d.maxPrice]} xDomain={[0,252]} xTicks={[0,63,126,189,252]} xLabel="วันซื้อขายจำลอง" yLabel="ราคาหุ้น (ดอลลาร์)" lines={d.visible.map((p,n)=>({values:p.map((v,i)=>[i,v]),color:['#6200ee','#00796b','#1976d2','#d84315','#c2185b','#827717','#6d4c41','#00838f','#7b1fa2','#388e3c'][n%10],width:1.1,opacity:.72}))}/>
    <p className="lab-note">แต่ละสีคือเส้นทางที่สุ่มได้ ไม่ใช่สินทรัพย์คนละตัว · แสดง {d.visible.length} เส้นจากทั้งหมด {count.toLocaleString()} เส้น สถิติด้านล่างใช้ทุกเส้น · กราฟปรับแกนราคาตามช่วงที่จำลองได้</p>
    ...
    <p className="lab-note">เส้นสีม่วงคือค่าเฉลี่ยสะสม เส้นสีเขียวคือค่าเฉลี่ยตามสูตร · <a href="glossary.html#standard-error" className="glossary-link">Standard error</a> ของค่าเฉลี่ยประมาณ {format(d.se)} ดอลลาร์ เป็นความคลาดเคลื่อนจากการสุ่มคำนวณ ไม่ใช่ความเสี่ยงของหุ้น</p>
    ...
    <p className="lab-note">จำลองแบบ exact GBM บน 252 ช่วงเวลา โดย μ และ σ คงที่ ไม่มีเงินปันผล ใช้ seed เดิมซ้ำได้ เมื่อเพิ่มจำนวนรอบจะคงเส้นทางเดิมไว้แล้วเพิ่มเส้นทางใหม่ · ใช้ μ เพื่อศึกษาราคา ยังไม่ใช่การกำหนดราคา Option แบบ risk-neutral</p>
  </div>;
}
