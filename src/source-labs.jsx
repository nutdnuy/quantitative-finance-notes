...
export function ReturnsLab(){
  ...
  <p className="lab-note">แกนเวลาเรียงตามจุดข้อมูล ไม่ได้แทรกวันหยุดให้เป็น step เท่ากัน · ราคาพิมพ์เพียง 2 ตำแหน่ง ผลตอบแทนที่คำนวณกลับจึงอาจต่างจากคอลัมน์เดิม · ตัวอย่างช่วงสั้นนี้ไม่ใช่อนุกรมเต็มที่ผู้เขียนใช้รายงานสถิติ</p>
...
export function StepLab(){
  ...
  return <div className="lab"><LabTitle number="3.8" title="สร้างราคาทีละ step แล้วดูว่าแต่ละเทอมเติมอะไร">พารามิเตอร์ตามรูป 3.10: S₀ = 100, μ = 15%, σ = 25%, δt = 0.01 ปี · สุ่มใหม่เพื่อสาธิต</LabTitle>
  <Range label="step ที่กำลังดู" value={index} onChange={setIndex} min={1} max={100} suffix=" / 100" />
  <Chart title="เส้นราคาที่สร้างด้วยสูตร random walk แบบไม่ต่อเนื่อง" description={`step ที่ ${index} ราคาเดิม ${format(term.current)} drift ${format(term.drift)} ช็อก ${format(term.shock)} ราคาถัดไป ${format(term.next)}`} xDomain={[0,1]} yDomain={[0,max]} xLabel="เวลา (ปี)" yLabel="ราคาจำลอง" lines={[{values:path.prices.map((v,i)=>[i*.01,v]),className:'sample-line'},{values:active.map((v,i)=>[i*.01,v])}]} markers={[{x:index*.01,y:term.next}]} />
  ...
  <p className="lab-note">เลขสุ่ม step นี้ φ = {format(term.phi,4)} · drift = 0.15 × {format(term.current,4)} × 0.01 · ช็อก = 0.25 × {format(term.current,4)} × {format(term.phi,4)} × 0.1</p>
  <div className="lab-actions"><button onClick={()=>setIndex(Math.min(100,index+1))} disabled={index===100}>ดูอีกหนึ่ง step</button><button onClick={()=>{setSeed(seed+1);setIndex(1);}}>สุ่มเส้นใหม่</button><span className="seed">seed {seed}</span></div>
  </div>;
}
...
export function WienerLab(){
  ...
  return <div className="lab"><LabTitle number="3.9" title="step สั้นลง แต่ความสุ่มตลอดหนึ่งปียังอยู่">รวม increments จากเส้นละเอียดเดียวกัน ทำให้จุดร่วมและจุดปลายตรงกันเมื่อเปลี่ยนจำนวน step</LabTitle>
  <div className="segmented" aria-label="จำนวน step ของ Wiener process">{levels.map((n,i)=><button key={n} aria-pressed={i===level} onClick={()=>setLevel(i)}>{n} step</button>)}</div>
  <Chart title="Wiener process เส้นเดียวกันที่ความละเอียดต่างกัน" description={`แบ่งหนึ่งปีเป็น ${steps} step SD ของ increment ${format(d.incrementSD,4)} ความแปรปรวนรวมตามทฤษฎี 1`} xDomain={[0,1]} yDomain={[-extent,extent]} xLabel="เวลา (ปี)" yLabel="Xₜ" lines={[{values:d.fine.map((v,i)=>[i/1024,v]),className:'sample-line'},{values:d.coarse.map((v,i)=>[i/steps,v])}]} markers={[{x:1,y:d.coarse.at(-1)}]} yFormat={v=>format(v,1)} />
  <Chart compact title="increments ของ Wiener process" description="แท่งแสดงการเปลี่ยนแปลงในแต่ละ step ค่าบวกและลบสะสมเป็นเส้นด้านบน" xDomain={[0,1]} yDomain={[-bound,bound]} xLabel="เวลา (ปี)" yLabel="การเปลี่ยนแปลง ΔX" yFormat={v=>format(v,2)} bars={d.increments.map((v,i)=>({x:i/steps,width:1/steps,y:v,className:v<0?'negative-bar':'positive-bar'}))} />
  <div className="results three"><div><span>ขนาด step δt</span><strong>{format(d.dt,4)}</strong><p>1 / {steps} ปี</p></div><div><span>SD ต่อ step √δt</span><strong>{format(d.incrementSD,4)}</strong><p>ย่อเมื่อเพิ่มจำนวน step</p></div><div><span>ความแปรปรวนรวม 1 ปี</span><strong>1</strong><p>{steps} × {format(d.dt,6)}</p></div></div>
