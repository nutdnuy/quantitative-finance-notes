...
export function Chart({ title, description, xDomain = [0, 1], yDomain = [0, 200], xLabel, yLabel, lines = [], bars = [], band, markers = [], verticals = [], xTicks, xFormat = v => Number(v.toFixed(2)), yFormat = v => format(v, 0), compact = false }) {
  ...
  const height = compact ? 215 : width < 450 ? 265 : 320, left = 51, right = 20, top = 30, bottom = 48;
  ...
  return <div className="chart" ref={ref}><svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} role="img" aria-labelledby={`${titleId} ${descId}`}>
    ...
    {lines.map((line, i) => <polyline key={i} points={points(line.values)} fill="none" className={line.className || 'primary-line'} strokeWidth={line.width || 2} />)}
