...
export default function BinomialLab() {
  ...
  return <div className="lab binomial-lab">
    <LabTitle number="เพิ่มเติม" title="เริ่มจาก payoff แล้วคิดย้อนกลับทีละ step">หุ้นวันนี้ 100 ดอลลาร์ · อายุ Option 1 ปี · 2 step · step ละ ½ ปี · European ใช้สิทธิได้เมื่อหมดอายุ</LabTitle>
    ...
    <div className="controls two">
      ...
      <Range label="หุ้นขึ้นต่อ step (u − 1)" value={upPercent} onChange={setUpPercent} min={5} max={30} suffix="%" />
      <Range label="หุ้นลงต่อ step (1 − d)" value={downPercent} onChange={setDownPercent} min={5} max={30} suffix="%" />
    </div>
    <p className="lab-note">u = {format(1 + upPercent / 100, 2)} · d = {format(1 - downPercent / 100, 2)} · Δt = 0.5 ปี · R = 1 + rΔt = {format(1 + rate / 100 * 0.5, 3)} ต่อ step<br />
      u และ d เป็นสมมติฐานต่อ step สำหรับเรียนรู้ ยังไม่ได้ประมาณจากข้อมูลตลาดหรือปรับตาม volatility ในแต่ละ step ใช้ดอกเบี้ยอย่างง่าย rΔt แล้วทบต้นข้าม step ด้วย R²</p>
    {calculated.error ? <p className="reading-note" role="alert"><strong>ยังคำนวณราคาแบบไม่มีอาร์บิทราจไม่ได้</strong><br />{calculated.error} ลองปรับดอกเบี้ยหรือขนาดการขึ้นลงให้ d &lt; R &lt; u</p> : <>
      ...
      <p className="hedge-explanation" aria-live="polite">{stage === 0
        ...
        : stage === 1
          ? 'ที่เวลา ½ ปี แต่ละจุดเหลืออีก 1 step: เฉลี่ย payoff ปลายทางด้วย q และ 1 − q แล้วหารด้วย R เพื่อคิดลดกลับมาที่จุดนั้น'
      ...
      <div className="table-wrap" tabIndex="0" role="region" aria-label={`ตาราง Binomial เวลา ${n / 2} ปี เลื่อนแนวนอนได้`}>
        <table>
          <caption>เวลา t = {format(n / 2, 1)} ปี · {stage === 0 ? 'มูลค่าเมื่อหมดอายุคือ payoff' : 'V คือมูลค่า Option ณ เวลาของแถว; Δ และ B ใช้จำลอง step ถัดไป'}</caption>
      ...
      <div className="step-footer"><span>ขั้น {stage + 1} / 3</span><div>
        ...
        <button disabled={stage === 2} onClick={() => setStage(value => value + 1)}>คิดย้อนกลับอีก step</button>
      </div></div>
      <div className="results" aria-live="polite">
        <div><span>ราคา {optionName} วันนี้จากแบบจำลอง</span><strong data-binomial="price">{format(model.price, 4)} <small>ดอลลาร์</small></strong><p>คิดลดจาก payoff ด้วย q ย้อนกลับ 2 step</p></div>
        ...
        <div><span>หุ้นในพอร์ตจำลองวันนี้ Δ</span><strong data-binomial="delta">{format(model.delta, 4)} <small>หุ้น</small></strong><p>{model.delta < 0 ? 'ค่าติดลบหมายถึงขายชอร์ตหุ้น' : 'จำนวนหุ้นต่อ Option 1 หน่วย'} · ปรับพอร์ตใหม่เมื่อถึง step ถัดไป</p></div>
      ...
      <div className="hedge-experiment">
        ...
        <p className="lab-note">สมมติ p คงที่และการขึ้นลงแต่ละ step เป็นอิสระ จึงให้น้ำหนักปลายทางเป็น p², 2p(1 − p), (1 − p)² แล้วหารด้วย R² การเปลี่ยน p เปลี่ยนค่าคาดหมายนี้ แต่ราคาจากการจำลอง payoff ใช้ q จึงไม่เปลี่ยนเมื่อ S, K, u, d และ r คงเดิม</p>
      </div>
    </>}
    <div className="lab-actions"><button onClick={reset}>คืนค่าเริ่มต้น</button></div>
    <p className="lab-note">สมมติหุ้นไม่จ่ายเงินปันผล ไม่มีค่าธรรมเนียมหรือภาษี ซื้อขายหุ้นเศษส่วนและขายชอร์ตได้ กู้และฝากได้ที่อัตราเดียวกัน และปรับพอร์ตได้ทุก step ผลลัพธ์เป็นราคาภายใต้แบบจำลองนี้</p>
  </div>;
}
