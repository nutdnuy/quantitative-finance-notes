...
export function QuadraticVariationLab() {
  ...
  return <div className="lab quadratic-variation-lab">
    <LabTitle number="เพิ่มเติม" title="ผลรวมกำลังสองที่เข้าใกล้เวลา และผลรวมแบบ Itô">เวลา T = 1 หน่วยเวลา · สร้าง Brownian increments บนกริดละเอียด 4,096 ก้าว แล้วรวมก้าวย่อยเมื่อเปลี่ยน N</LabTitle>
    <div className="segmented" role="group" aria-label="จำนวนก้าวสำหรับ Quadratic variation">{[16, 64, 256, 1024, 4096].map(n => <button key={n} aria-pressed={steps === n} onClick={() => setSteps(n)}>{format(n, 0)} ก้าว</button>)}</div>
    ...
    <div className="results" aria-live="polite">
      ...
      <div><span>ผลรวมซ้าย Σ Wก่อนก้าว × ΔW</span><strong data-qv="ito-left">{format(data.itoLeft, 6)}</strong><p>หน่วยเวลา · ใช้ค่า W ก่อนเห็น increment ของก้าวนั้น</p></div>
    ...
    <div className="calculation-strip"><span>เอกลักษณ์บนกริดจำกัด</span><strong>Σ Wก่อนก้าว × ΔW = ½(Wₜ² − Qₙ) = {format(data.finiteIdentity, 6)}</strong><p className="lab-note">Wₜ = <output data-qv="terminal">{format(data.terminal, 6)}</output> · ความต่างจากเอกลักษณ์เชิงพีชคณิต <output data-qv="identity-error">{data.identityError.toExponential(2)}</output> (การปัดเศษของคอมพิวเตอร์)</p></div>
    ...
    <p className="lab-note">กราฟเชื่อมค่าที่จุดบนกริดเพื่อให้อ่านง่าย การทดลองนี้ใช้ Gaussian increments ที่สุ่มได้ ซึ่งต่างจากการเดินด้วยก้าว ±√Δt ที่แต่ละก้าวมีกำลังสองเท่ากับ Δt พอดี</p>
  </div>;
}
...
export function GbmEulerLab() {
  ...
  return <div className="lab gbm-euler-lab">
    <LabTitle number="เพิ่มเติม" title="เปรียบเทียบ Exact GBM กับ Euler บนความสุ่มชุดเดียวกัน">S₀ = 100 ดอลลาร์ · T = 1 ปี · μ และ σ คงที่ · ทั้งสองวิธีใช้ Brownian increments เดียวกันทุกก้าว</LabTitle>
    ...
    <div className="segmented" role="group" aria-label="จำนวนก้าวสำหรับ Exact และ Euler">{[4, 16, 64, 256, 1024].map(n => <button key={n} aria-pressed={steps === n} onClick={() => setSteps(n)}>{format(n, 0)} ก้าว</button>)}</div>
    ...
    <div className="results" aria-live="polite">
      ...
      <div><span>Euler ที่ปลายปี</span><strong data-gbm-euler="euler">{format(data.euler.at(-1), 4)} <small>ดอลลาร์</small></strong><p>ประมาณทีละก้าว ขนาด Δt = {format(data.dt, 6)} ปี</p></div>
...
export function CorrelationLab() {
  ...
  return <div className="lab correlation-lab">
    ...
    <p className="lab-note">Sample correlation อาจต่างจากค่าเป้าหมายเพราะมีตัวอย่างจำกัด กราฟนี้เปรียบเทียบช็อกของแต่ละก้าว ไม่ใช่ correlation ของระดับราคาตลอดเส้นทาง และยังไม่ใช่ค่าประมาณจากข้อมูลตลาด</p>
  </div>;
}
