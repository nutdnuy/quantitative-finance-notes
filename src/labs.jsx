...
function JensenLab() {
  ...
  return <div className="lab"><LabTitle number="3.3" title="ทำไมจุดเฉลี่ยจึงอยู่เหนือเส้น payoff">ตัวอย่างต้นฉบับ: ราคาเฉลี่ย 100 ดอลลาร์ · โอกาสของสองกรณีเท่ากัน</LabTitle>
    <div className="controls two"><Range label="ระยะห่างจากราคาเฉลี่ย" value={spread} onChange={setSpread} min={0} max={90} suffix=" ดอลลาร์" /><Range label="ราคาใช้สิทธิ K" value={strike} onChange={setStrike} min={60} max={160} suffix=" ดอลลาร์" /></div>
    <Chart title="Jensen: เฉลี่ยบนเส้นเชื่อม เทียบกับ payoff ที่ค่าเฉลี่ย" description={`ราคาสองกรณี ${d.low} และ ${d.high} ค่าเฉลี่ย payoff ${d.expected} เทียบกับ payoff ที่ราคาเฉลี่ย ${d.atMean} ดอลลาร์`} xDomain={[0, 220]} yDomain={[0, 160]} xTicks={[0,50,100,150,200]} xLabel="ราคาปลายปี (ดอลลาร์)" yLabel="Payoff (ดอลลาร์)" lines={[{ values: [[0, 0], [strike, 0], [220, 220 - strike]] },{values:[[d.low,d.lowPay],[d.high,d.highPay]],className:'secondary-line'},{values:[[100,d.atMean],[100,d.expected]],className:'gap-line',width:3}]} markers={[{ x: d.low, y: d.lowPay }, { x: d.high, y: d.highPay }, { x: 100, y: d.atMean, className: 'mean-point', r: 5 },{x:100,y:d.expected,className:'expected-point',r:6}]} />
    ...
    <div className="results"><div><span>จุดบน: เฉลี่ย payoff</span><strong><CountUp to={d.expected} /> <small>ดอลลาร์</small></strong><p>½ × {d.lowPay} + ½ × {d.highPay}</p></div><div><span>จุดล่าง: payoff ที่ราคาเฉลี่ย</span><strong>{format(d.atMean)} <small>ดอลลาร์</small></strong><p>max(100 − {strike}, 0)</p></div></div>
...
function GbmLab() {
  ...
  return <div className="lab gbm"><LabTitle number="เพิ่มเติม" title="มองหลายเส้นทางพร้อมกัน">GBM แบบ exact · ราคาเริ่มต้น 100 ดอลลาร์ · ระยะเวลา 1 ปี · 252 ก้าว · 40 เส้นทาง</LabTitle>
