...
[Trinomial random walk](glossary.html#trinomial-random-walk) ในเอกสารนี้ให้ตัวแปรเลือกได้สามทางในก้าวเวลา Δt

| การเคลื่อนที่ | ค่าหลังหนึ่งก้าว | ความน่าจะเป็น |
|---|---|---:|
| ขึ้น | y + h | α |
| อยู่นิ่ง | y | 1 − 2α |
| ลง | y − h | α |

h คือขนาดก้าวในหน่วย y และ α คือความน่าจะเป็นที่ไม่มีหน่วย เราใช้ **0 < α < 1/2** เพื่อให้ทั้งสามทางมีโอกาสเกิด และสมมติว่าการเลือกแต่ละก้าวเป็นอิสระ ใช้ α และ h ชุดเดิมทุกก้าว

เมื่อตั้ง α = 0.2, h = 1 และ Δt = 1 จะได้โอกาสขึ้น 20% อยู่นิ่ง 60% ลง 20% ถ้าเริ่ม y = 0 หลังสองก้าวจะเป็นดังนี้

| ค่าหลังสองก้าว | −2 | −1 | 0 | 1 | 2 |
|---|---:|---:|---:|---:|---:|
| ความน่าจะเป็น | 0.04 | 0.24 | 0.44 | 0.24 | 0.04 |

ทำไมจุด 0 มีโอกาส 0.44? เพราะมาถึงได้จากขึ้นแล้วลง ลงแล้วขึ้น หรืออยู่นิ่งทั้งสองก้าว จึงรวมเป็น **0.2 × 0.2 + 0.2 × 0.2 + 0.6 × 0.6 = 0.44** ค่าทั้งห้ารวมกันเป็น 1 พอดี
...
เมื่อก้าวละเอียดขึ้น การเปรียบเทียบที่เหมาะสมคือมวลความน่าจะเป็นของช่องกับ **พื้นที่ใต้โค้งในช่อง** หรือแปลงมวลเป็นความสูง m/h ก่อนเทียบรูปความหนาแน่น ไม่เอา m ไปเทียบกับ k โดยตรง

</section>

<section id="scaling">

## 3. ย่อก้าวอย่างไรให้ความสุ่มไม่หายไป

จากความสมมาตรของก้าวขึ้นและลง ค่าเฉลี่ยของการเปลี่ยนแปลงหนึ่งก้าวเป็นศูนย์ ส่วนความแปรปรวนเป็น
...
เมื่อเดิน N ก้าวที่เป็นอิสระ ความแปรปรวนบวกกันได้ ถ้า τ = NΔt จะได้
...
ในตัวอย่าง α = 0.2, h = 1, Δt = 1 จะมี c² = 0.2 ความแปรปรวนหลังสองก้าวจึงเป็น **2 × 0.2 × 2 = 0.8** ตรงกับตาราง แต่ตารางสองก้าวยังเป็นการแจกแจงไม่ต่อเนื่อง แม้จะมีค่าเฉลี่ยและความแปรปรวนตรงกับ Normal ที่นำมาเปรียบเทียบก็ตาม
...
ตรึงจุดเริ่มต้น y,t ไว้ แล้วถามว่า “ความน่าจะเป็นที่จุด z ในก้าวถัดไป มาจากจุดใดบ้าง”

ให้ mₙ(j) เป็นมวลความน่าจะเป็นที่ตำแหน่ง y + jh หลัง n ก้าว จุดนี้รับมวลมาจากเพื่อนบ้านด้านล่างที่เดินขึ้น จากตัวเองที่อยู่นิ่ง และจากเพื่อนบ้านด้านบนที่เดินลง
...
ตัวอย่างมวลที่ 0 หลังสองก้าวคือ 0.2 × 0.2 + 0.6 × 0.6 + 0.2 × 0.2 = 0.44 นี่คือการเดิน **การแจกแจงทั้งชุดไปข้างหน้า** โดยไม่ต้องสุ่มเส้นทางทีละเส้น
...
เมื่อนำไปแทนในสูตรอัปเดต พจน์ที่เป็น h จะหักล้างกันเพราะขึ้นและลงมีน้ำหนักเท่ากัน เหลือ Δt k_T ≈ αh²k_zz เมื่อให้ก้าวเล็กลงตามความสัมพันธ์ในหัวข้อก่อน จะได้
...
## 5. Backward: แยกว่าก้าวแรกพาไปไหนได้บ้าง

คราวนี้ตรึงเป้าหมายในอนาคตและเวลา T ไว้ แล้วถามว่า “ถ้าเริ่มจาก y ณ เวลา t โอกาสไปถึงเป้าหมายนั้นเป็นเท่าไร”

เราใช้ **[Markov property](glossary.html#markov-property)**: เมื่อรู้สถานะปัจจุบันแล้ว ประวัติที่เดินมาก่อนหน้าไม่เพิ่มข้อมูลที่จำเป็นต่อการแจกแจงของอนาคตในแบบจำลองนี้ จึงแยกตามก้าวถัดไปแล้วรวมความน่าจะเป็นได้ ให้ Mₕ(y,t;z,T) เป็นมวลความน่าจะเป็นที่จะจบที่จุดตาราง z เมื่อเริ่มจาก y
...
นี่คือความสัมพันธ์ที่แน่นอนบนตาราง เมื่อหารมวลด้วย h แล้วใช้ความหนาแน่นที่เรียบเป็นรูปประมาณ เราจึงนำไปขยาย Taylor เพื่อหาสมการในลิมิต ไม่ได้อ้างว่า Gaussian ต่อเนื่องผ่านสูตรสามกิ่งนี้อย่างพอดีสำหรับก้าวที่ยังมีขนาดจำกัด

ลองใช้เป้าหมายว่า **หลังสองก้าวต้องอยู่ที่ 0** จากจุดเริ่มต้น 0 เมื่อ α = 0.2 และ h = 1

| จุดที่ไปถึงหลังก้าวแรก | โอกาสไปถึงจุดนั้น | โอกาสก้าวที่เหลือจบที่ 0 |
|---|---:|---:|
| 1 | 0.2 | 0.2 |
| 0 | 0.6 | 0.6 |
| −1 | 0.2 | 0.2 |

เฉลี่ยคอลัมน์สุดท้ายด้วยน้ำหนักก้าวแรก จึงได้ **0.2 × 0.2 + 0.6 × 0.6 + 0.2 × 0.2 = 0.44** เหมือน Forward แต่ครั้งนี้เรามองจากจุดเริ่มต้นไปยังมูลค่าความน่าจะเป็นที่ทราบสำหรับก้าวถัดไป

เมื่อขยาย Taylor รอบ **y,t** พจน์เวลาก้าวถัดไปมีเครื่องหมายบวก และพจน์อันดับหนึ่งของ h หักล้างกัน จึงได้
...
![Forward รวมมวลจากสามสถานะที่เวลาก่อนหน้าเข้าสู่จุดปลายทางเดียว ส่วน Backward แยกจากจุดเริ่มต้นไปสามสถานะในก้าวถัดไป โดยตรึงเป้าหมายในอนาคตไว้](assets/diagrams/kolmogorov-directions.svg)

</div>

<p class="figure-caption">ภาพแสดงวิธีจัดความสัมพันธ์ของก้าว ไม่ใช่เส้นทางย้อนเวลา · <a href="assets/diagrams/kolmogorov-directions.svg">เปิดภาพขนาดเต็ม</a> · <a href="assets/diagrams/kolmogorov-directions.excalidraw" download>ดาวน์โหลดภาพที่แก้ไขได้</a></p>
...
## 10. ทดลองกับพื้นที่ใต้โค้งและก้าวที่ละเอียดขึ้น
...
### เพิ่มจำนวนก้าว โดยคงความแปรปรวนปลายทางไว้
...
การคำนวณแท่งใช้การรวมความน่าจะเป็นทุกก้าวโดยตรง จึงไม่มี Monte Carlo sampling error ความต่างจากโค้งเกิดจากก้าวที่ยังมีขนาดจำกัดและการประมาณลิมิต การเพิ่ม N ภายใต้สมมติฐานที่คุมไว้ช่วยให้เห็นการเข้าใกล้คำตอบต่อเนื่อง แต่ไม่ได้เป็นหลักฐานว่าแบบจำลองนี้อธิบายตลาดจริงได้ดี
...
ดาวน์โหลด [Notebook ของบท Transition Density Functions](notebooks/transition-density-functions.ipynb) เพื่อรันตารางความน่าจะเป็น ตรวจพื้นที่ใต้ Gaussian และทดลองความละเอียดของก้าวด้วย Python
...
1. ก้าวขึ้นและลงมีโอกาสอย่างละ 0.2 ขนาดก้าว 1 และเป็นอิสระ ถ้าเดิน 100 ก้าว ค่าเฉลี่ยการเปลี่ยนแปลงและ SD เท่าไร
...
สิ่งที่ควรเก็บกลับไปคือ เมื่อมองความน่าจะเป็นทั้งการแจกแจง กฎการเดินสุ่มเล็ก ๆ สามารถเขียนเป็นสมการการแพร่ได้ ความสัมพันธ์ระหว่างก้าวตำแหน่งและก้าวเวลากำหนดขนาดการกระจาย ส่วนเงื่อนไขเริ่มต้นและขอบเขตกำหนดว่าเรากำลังหาคำตอบแบบใด
...
- เรียบเรียงภาษาไทยใหม่ ใช้ **k(y,t;z,T)** แทน p(y,t;y′,t′), h แทน δy และ τ = T − t แยกมวลความน่าจะเป็นบนตารางจากความหนาแน่นต่อเนื่องอย่างชัดเจน ตัวอย่างตารางสองก้าว พื้นที่ช่วง 0 ถึง 2 แบบฝึกหัด ภาพ และห้องทดลองเป็นส่วนที่คำนวณขึ้นสำหรับบทนี้
