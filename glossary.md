...
แบบจำลองเวลาที่แบ่งเป็นก้าว โดยราคาหุ้น ณ แต่ละจุดไปต่อได้สองค่า เช่น คูณด้วย u หรือ d การกำหนดราคา Option ใช้พอร์ตหุ้นร่วมกับเงินฝากหรือเงินกู้ที่เลียนแบบมูลค่าสัญญาในสองกิ่งได้ ภายใต้สมมติฐาน no arbitrage
...
ต้นไม้ที่เส้นทางต่างกันมาถึงสถานะเดียวกันได้ เช่น ขึ้นแล้วลงกับลงแล้วขึ้นให้ราคา S₀ud เท่ากัน เมื่อใช้ตัวคูณ u และ d คงที่ จึงมีราคาปลายทาง N + 1 จุดหลัง N ก้าว การรวมกิ่งไม่จำเป็นต้องมี ud = 1
...
เริ่มจากมูลค่าที่รู้ ณ ปลายทาง แล้วใช้ค่านั้นหามูลค่าในก้าวก่อนหน้า ทำซ้ำจนถึงวันนี้ สำหรับ European Option เริ่มจาก payoff วันหมดอายุ ใช้น้ำหนัก q และ 1 − q แล้วคิดลดทีละก้าว
...
การเปลี่ยนสัดส่วนสินทรัพย์เมื่อเวลาและราคาหุ้นเปลี่ยนไป เช่น ในต้นไม้ Binomial ต้องคำนวณ Delta ใหม่ ณ แต่ละจุดเพื่อเลียนแบบมูลค่า Option ในก้าวถัดไป สัดส่วนเดิมที่เลียนแบบได้หนึ่งก้าวไม่จำเป็นต้องใช้ได้จนหมดอายุ
...
แบบจำลองที่สถานะเปลี่ยนไปทีละก้าวตามผลสุ่ม เช่น นำการเปลี่ยนแปลงสุ่มแต่ละช่วงมาสะสมเป็นเส้นทาง คำนี้ครอบคลุมแบบจำลองได้หลายแบบ จึงต้องดูว่ากติกาแต่ละก้าวคืออะไร ในบทนี้เราต่อยอดไปสู่ Wiener process และการจำลองราคาด้วย GBM
...
การเดินสุ่มที่ในหนึ่งก้าวเลือกได้สามการเคลื่อนที่ บทนี้ใช้ขึ้น h อยู่นิ่ง หรือลง h ด้วยโอกาส α, 1 − 2α และ α ตามลำดับ ก้าวแต่ละก้าวเป็นอิสระและใช้พารามิเตอร์คงที่ เป็นแบบจำลองตัวแปรบนเส้นจำนวน ไม่ใช่ต้นไม้สำหรับกำหนดราคา Option โดยอัตโนมัติ
...
ความน่าจะเป็นที่ตัวแปรไม่ต่อเนื่องมีค่าใดค่าหนึ่ง เช่น เดินสุ่มสองก้าวแล้วอยู่ที่ 0 มีโอกาส 0.44 มวลทุกค่ารวมกันเป็น 1 ต่างจากความหนาแน่นต่อเนื่องซึ่งต้องอินทิเกรตเหนือช่วงก่อนจึงได้ความน่าจะเป็น
...
เมื่อรู้สถานะปัจจุบันแล้ว การแจกแจงแบบมีเงื่อนไขของอนาคตไม่ต้องอาศัยประวัติก่อนหน้านั้นเพิ่มเติม ภายใต้แบบจำลองที่กำหนด สมบัตินี้ทำให้แยกคำนวณตามก้าวถัดไปได้ ไม่ใช่ข้ออ้างว่าข้อมูลอดีตไม่มีประโยชน์ในตลาดจริงทุกกรณี
...
การประมาณฟังก์ชันใกล้จุดหนึ่งด้วยค่าฟังก์ชันและอนุพันธ์ที่จุดนั้น เช่น เก็บพจน์อันดับหนึ่งของเวลาและอันดับสองของตำแหน่งเพื่อเชื่อมกฎการเดินสุ่มกับสมการการแพร่ การตัดพจน์ที่สูงกว่าทิ้งเป็นการประมาณ ต้องพิจารณาขนาดก้าวและความเรียบของฟังก์ชัน
