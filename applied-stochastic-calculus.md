...
|---|---|---|
| 1. มองก้าวเล็ก ๆ | ทำไมก้าว Brownian ย่อไม่เร็วเท่าก้าวเวลา | Quadratic variation |
...
## 1. ก้าวเวลาย่อครึ่งหนึ่ง แต่ขนาดช็อกไม่ได้ย่อครึ่งหนึ่ง
...
ถ้าบีบ n ครั้งให้อยู่ในเวลารวม T และลดขนาดแต่ละก้าวเป็น ±√(T/n) จะมี
...
การเลือก **ขนาดก้าวตาม √Δt** ทำให้ความสุ่มไม่หายไปเมื่อแบ่งเวลาละเอียดขึ้น ลิมิตของการเดินสุ่มที่ปรับสเกลเหมาะสมนี้ให้ Brownian motion ส่วนในแบบจำลอง Brownian โดยตรง ก้าวในช่วงเวลาที่มีขนาดจำกัดมีการแจกแจง
...
Brownian motion มีเส้นทางต่อเนื่อง ก้าวในช่วงเวลาที่ไม่ทับกันเป็นอิสระ และ SD ของก้าวเท่ากับ √Δt สัญลักษณ์ **dW** ใช้ในสมการเชิงสุ่ม ไม่ใช่อนุพันธ์ธรรมดาที่เรานำไปหาร dt แล้วได้ความเร็วของเส้นทางที่เรียบ
...
## 2. ก้าวเล็กลง แต่ผลรวมกำลังสองยังเหลืออยู่
...
| ก้าวที่ใช้ | กำลังสองของก้าวเดียว | ผลรวม Qₙ |
|---|---|---|
| เหรียญ ±√Δt | เท่ากับ Δt ทุกครั้ง | เท่ากับ T พอดี แม้ n ยังจำกัด |
| ก้าว Gaussian ของ Brownian | Δt Zᵢ² ยังเป็นตัวแปรสุ่ม | ยังสุ่มที่ n จำกัด แต่เข้าใกล้ T ในลิมิต |
...
**กฎนี้เป็นสัญลักษณ์ของการคำนวณในลิมิต ไม่ใช่สมการของก้าว Gaussian แต่ละก้าว** ที่ Δt ยังไม่เป็นศูนย์ เราต้องใช้ (ΔW)² = Δt Z² ซึ่งไม่เท่ากับ Δt เสมอ
...
เริ่มด้วยเอกลักษณ์พีชคณิตที่ถูกต้องสำหรับทุกก้าว
...
รวมทุกก้าวและใช้ W₀ = 0 จะได้

$$
W_T^2=2\sum_{i=0}^{n-1}W_{t_i}\Delta W_i+Q_n.
$$

ผลรวมตรงกลางใช้ค่า W **ก่อน** รับช็อกของช่วงถัดไป นี่เป็นจุดเริ่มต้นของ **[Itô integral](glossary.html#ito-integral)** ซึ่งใช้ข้อมูลที่มีอยู่แล้ว ไม่เอาค่าในอนาคตมาย้อนกำหนดน้ำหนักของก้าวปัจจุบัน
...
### ทดลอง: ดูพจน์ที่สะสมจากกำลังสองของก้าว

ห้องทดลองใช้ Gaussian increments ของ Brownian path เดียวกันบนตารางละเอียด แล้วรวมก้าวติดกันเมื่อเลือก n ที่เล็กลง จึงเปรียบเทียบความละเอียดโดยไม่เปลี่ยนเส้นทางสุ่มพื้นฐาน
...
เมื่อ r ต่ำกว่า θ พจน์ drift เป็นบวก เมื่อ r สูงกว่า θ drift เป็นลบ แต่ช็อกยังอาจผลักให้เดินออกห่างได้ **mean reversion เป็นแรงดึงใน drift ไม่ใช่คำสัญญาว่าทุกก้าวต้องเข้าใกล้ค่าเฉลี่ย**
...
วิธี **[Euler–Maruyama](glossary.html#euler-maruyama)** ประมาณการเปลี่ยนแปลงโดยประเมินสัมประสิทธิ์ที่ต้นช่วง แล้วใช้ช็อกใหม่ในแต่ละก้าว:
...
คำว่า exact ในที่นี้หมายถึงคำตอบของ **GBM ที่สมมติไว้ ณ จุดเวลาที่คำนวณ** เส้นเชื่อมจุดบนกราฟเป็นภาพช่วยอ่าน ไม่ใช่การแสดงรายละเอียดทุกก้าวของเส้นทางต่อเนื่อง และคำตอบ exact ของแบบจำลองไม่ได้รับประกันความถูกต้องของแบบจำลองต่อโลกจริง
...
|---|---|---|
| [การแบ่งเวลา](glossary.html#discretization-error) | แทน SDE ด้วยก้าวที่ยังมีขนาดจำกัด | ย่อ Δt หรือเลือกวิธีเหมาะสม |
...
ในการจำลองตามเวลา ให้สุ่มคู่ Z₁,Z₂ ใหม่อย่างอิสระจากคู่ในช่วงก่อนหน้า เมื่อแปลงเป็นก้าว Brownian ด้วย ΔW₁ = √Δt φ₁ และ ΔW₂ = √Δt φ₂ จะมี Cov(ΔW₁,ΔW₂) = ρΔt หรือเขียนย่อว่า
...
1. ถ้า Δt = 0.01 และ Z = 2 ก้าว ΔW และกำลังสองของก้าวเป็นเท่าไร กฎ (dW)² = dt บอกว่าค่านี้ต้องเท่ากับ 0.01 หรือไม่
...
4. ใน OU ถ้า r สูงกว่า θ จะสรุปได้หรือไม่ว่าก้าวถัดไป r ต้องลด
...
1. ΔW = √0.01 × 2 = **0.2** และ (ΔW)² = **0.04** กฎเชิง differential ไม่ใช่การบังคับให้กำลังสองของก้าวที่มีขนาดจำกัดเท่ากับ Δt
...
- <em>CQF Module 1 Lecture 4 — Introduction to Stochastic Calculus</em>, ไฟล์ที่ผู้ใช้ให้ **JA251.4 Notes.pdf**, 43 หน้า; หน้า 2 ใช้ชื่อ <em>Stochastic Calculus and Itô’s Lemma</em> ไม่ปรากฏชื่อผู้เขียนรายบุคคลในหน้าที่ตรวจ อ้างเลขหน้า PDF: 5–7 เหรียญและค่าคาดหมายแบบมีเงื่อนไข; 8–14 quadratic variation และ Brownian scaling; 15–28 ฟังก์ชันของ Brownian, Taylor และ Itô; 29–35 SDE, arithmetic Brownian และ GBM; 36–38 mean-square limit; 39–42 การสร้าง Normal
- <em>CQF Module 1 Lecture 5 — Stochastic Differential Equations – Maths and Computation</em>, ไฟล์ที่ผู้ใช้ให้ **JA251.5 Notes.pdf**, 44 หน้า; หน้า 2 ใช้ชื่อ <em>Simulating and Manipulating Stochastic Differential Equations</em> และระบุ Certificate in Quantitative Finance ไม่ปรากฏชื่อผู้เขียนรายบุคคลในหน้าที่ตรวจ เลขหน้า PDF มากกว่าเลขสไลด์ภายในหนึ่งหน้า อ้างเลขหน้า PDF: 5–11 SDE และ Itô; 12–14 log และ exact GBM; 15–16 OU/Vasicek; 17–23 transition density และ Forward; 24–26 stationary distribution; 27 Backward; 28–36 การจำลอง Euler; 37–43 correlated increments และผลรวม Normal
